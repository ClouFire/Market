...
$app->router->post('checkout/coupon', [CartController::class, 'editTotalPrice'])->middleware(["auth"]);
...
$app->router->post('checkout', [CartController::class, 'placeOrder'])->middleware(['auth']);
$app->router->post('contact', [HomeController::class, 'registerMessage']);
$app->router->get('orderSuccess', [CartController::class, 'orderSuccess'])->middleware(['auth']);
